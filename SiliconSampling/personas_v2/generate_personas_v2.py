"""
V2 Persona Generation Script

Generates personas with enhanced structure:
- persona_description (2-3 paragraphs, first person)
- behavioral_tendencies (5 concrete decision patterns)
- self_schema (6-8 first-person belief statements)
...
Usage:
    ...
    python generate_personas_v2.py --strategy random --sample-size 200 --no-append
    
    # Skip automatic cleaning of failed personas
    python generate_personas_v2.py --strategy ipip --sample-size 1000 --no-cleaning
"""
...
async def call_openai_api_async(client, system_message, prompt, model="gpt-4o-mini", max_retries=3):
    """
    Async call to OpenAI API to generate v2 persona
    ...
    Returns:
        Generated persona dict or None if failed
    """
...
async def generate_personas_batch(df, system_message, template, strategy, model="gpt-4o-mini", 
                                  batch_save_size=100, concurrent_requests=50, append=True, auto_clean=True):
    """
    Generate v2 personas using async batch processing
    ...
    for idx, row in df.iterrows():
        ...
        prompt = create_persona_prompt(template, demographics, ocean_scores, strategy, persona_id=int(idx))
    ...
    async def process_one(idx, demographics, ocean_scores, prompt):
        async with semaphore:
            result = await call_openai_api_async(client, system_message, prompt, model=model)
            if result is None:
                ...
                return {
                    ...
                    'persona_description': '[FAILED] API call failed after retries',
            ...
            return {
                'id': result.get('persona_id', idx),
                'demographics': demographics,
                'ocean_scores': ocean_scores,
                'persona_description': result.get('persona_description', ''),
                'behavioral_tendencies': result.get('behavioral_tendencies', {}),
                'self_schema': result.get('self_schema', [])
            }
...
def main(strategy='random', sample_size=100, model="gpt-4o-mini", concurrent_requests=20, append=True, auto_clean=True):
    """
    Main v2 persona generation pipeline
    ...
    print("\n" + "="*80)
    print("V2 PERSONA GENERATION PIPELINE")
    print("="*80)
    ...
    total_count = asyncio.run(generate_personas_batch(
        df, system_message, template, strategy, model, 
        concurrent_requests=concurrent_requests, append=append, auto_clean=auto_clean
    ))
    
    # Get output file path
    from pathlib import Path
    output_file = Path(f"{strategy}_matching") / f"personas_{strategy}_v2.jsonl"
    
    print("\n" + "="*80)
    print("✅ V2 PERSONA GENERATION COMPLETE")
    print("="*80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate v2 personas with behavioral tendencies and self-schema")
    ...
    main(
        ...
        auto_clean=not args.no_cleaning
    )
