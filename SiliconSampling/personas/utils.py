"""
Utility functions for persona generation pipeline
...
def load_ocean_data(strategy, sample_size=None):
    """
    Load the census data with OCEAN scores for a given strategy
    Optimized to only load necessary columns and sample efficiently
    ...
    # Define columns we actually need for persona generation
    if strategy == 'wpp':
    ...
    # If sampling, use a much faster approach
    if sample_size:
        # Read only metadata to get total rows
        parquet_file = pq.ParquetFile(data_file)
        total_rows = parquet_file.metadata.num_rows
        
        # Calculate random row indices to read
        row_indices = sorted(np.random.choice(total_rows, min(sample_size, total_rows), replace=False))
        
        # Read only selected rows and columns (MUCH faster)
        # Keep original index for traceability back to source data
        df = pd.read_parquet(data_file, columns=columns_to_read)
        df = df.iloc[row_indices]
        
        print(f"✓ Sampled {len(df):,} records from {data_file} (total: {total_rows:,})")
    else:
        # Read all rows but only needed columns
        df = pd.read_parquet(data_file, columns=columns_to_read)
...
def format_ocean_scores(row, strategy):
    ...
    if strategy == 'random' or strategy == 'ipip':
        # Numeric 0-10 scores (use title case column names for Random, IPIP)
        ocean_traits = ['Openness', 'Conscientiousness', 'Extraversion', 'Agreeableness', 'Neuroticism']
        scores = {trait: round(row[trait], 2) for trait in ocean_traits if trait in row}
    
    elif strategy == 'wpp':
        # Text-based 7-point scale (lowercase column names for WPP)
        ocean_traits = ['openness', 'conscientiousness', 'extraversion', 'agreeableness', 'neuroticism']
...
def create_persona_prompt(template, demographics, ocean_scores, strategy):
    """
    Create the full prompt for LLM persona generation
    
    Args:
        template: The prompt template with trait guidance and instructions
...
def save_personas(personas, strategy, output_dir=".", append=True):
    """
    Save generated personas to JSONL file (one JSON object per line)
    By default appends to existing file, or creates new file if it doesn't exist.
    ...
    # JSONL file path
    jsonl_file = output_dir / f"personas_{strategy}.jsonl"
    
    # Count existing records (if file exists and is readable)
    existing_count = 0
    if append:
        try:
            ...
            print(f"  Could not read existing file ({e}), creating new file: {jsonl_file}")
    else:
...
def clean_failed_personas(strategy, output_dir="."):
    ...
    # Overwrite file with only valid personas
